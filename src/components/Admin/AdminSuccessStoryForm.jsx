...
const AdminSuccessStoryForm = () => {
  ...
  useEffect(() => {
    ...
    loadStory({ id })
      .then((story) => {
        ...
        const matched = getSuccessStoryCategoryOptions([story, ...stories]).find(
          (option) => option.toLowerCase() === current.toLowerCase(),
        );
        if (matched) {
          setCategorySelect(matched);
          setCustomCategory("");
          setValues((prev) => ({ ...prev, category: matched }));
        } else {
  ...
  useEffect(() => {
    ...
    if (!matched) return;
    if (categorySelect === matched) return;
    if (
      categorySelect === SUCCESS_STORY_ADD_NEW_CATEGORY &&
      customCategory.trim().toLowerCase() !== matched.toLowerCase()
    ) {
      return;
    }
    setCategorySelect(matched);
